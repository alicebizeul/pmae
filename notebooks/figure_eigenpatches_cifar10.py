...
trainset = torchvision.datasets.CIFAR10(data_folder, transform=transform)
trainloader =  torch.utils.data.DataLoader(trainset, batch_size=10, shuffle=True, num_workers=1)
data_iter = iter(trainloader)
images, labels = next(data_iter)

c=np.load(f'{folder}/pc_matrix.npy').T
eigenvalues=np.load(f'{folder}/eigenvalues.npy')
ratio=np.load(f'{folder}/eigenvalues_ratio.npy')
mean = np.load(f'{folder}/mean_reshaped.npy')
std = np.load(f'{folder}/std_reshaped.npy')
...
start_component, stop_component, nb_components = find_threshold(ratio,0.96), find_threshold(ratio,1.0), eigenvalues.shape[0]
...
print(c.shape,ratio.shape)
np.save(f"{folder}/eigenvalues_ratio_reduced.npy",ratio)
np.save(f"{folder}/pc_matrix_reduced.npy",c)