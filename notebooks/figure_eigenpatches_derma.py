...
trainset = BloodMNIST(root=folder, transform=transform,size=64,split="train")
trainloader =  torch.utils.data.DataLoader(trainset, batch_size=10, shuffle=True, num_workers=1)
data_iter = iter(trainloader)
images, labels = next(data_iter)

c=np.load(f'{folder}/bloodmnist_pc_matrix.npy').T
eigenvalues=np.load(f'{folder}/bloodmnist_eigenvalues.npy')
ratio=np.load(f'{folder}/bloodmnist_eigenvalues_ratio.npy')
mean = np.load(f'{folder}/bloodmnist_mean_reshaped.npy')
std = np.load(f'{folder}/bloodmnist_std_reshaped.npy')
...
r_start, r_stop, nb_components = 0, 0.01, eigenvalues.shape[0]
...
r_start, r_stop, nb_components = 0.01, 0.05, eigenvalues.shape[0]
start_component = int(r_start*nb_components)
final_component = int(r_stop*nb_components)
...
start_component, stop_component, nb_components = find_threshold(ratio,0.999), find_threshold(ratio,1.0), eigenvalues.shape[0]
...
row=1
column=1
ax[row,column].imshow(np.transpose(reconstructed_image[1], (1,2,0)),cmap="gray")
ax[row,column].axis('off')
...
print(c.shape,ratio.shape)
np.save(f"{folder}/bloodmnist_eigenvalues_ratio_reduced.npy",ratio)
np.save(f"{folder}/bloodmnist_pc_matrix_reduced.npy",c)
