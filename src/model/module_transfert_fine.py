...
class ViTMAE_transfert_fine(pl.LightningModule):
    ...
    def shared_step(self, batch: Tensor, stage: str = "train", batch_idx: int =None):
        if stage == "train":
            ...
            self.log(f"final_{stage}_classifier_loss_{self.evaluated_epoch}_transfert_fine_{self.datamodule.name}", loss_ce, sync_dist=True)

            accuracy_metric = getattr(self, f"online_{stage}_accuracy")
            accuracy_metric(self.online_logit_fn(logits.squeeze()), y.squeeze())
            self.log(
                f"final_{stage}_accuracy_{self.evaluated_epoch}_transfert_fine_{self.datamodule.name}",
                accuracy_metric,
                prog_bar=False,
                sync_dist=True,
            )
            ...
            if (self.current_epoch+1)%10==0 and batch_idx==0:
                plot_loss(self.avg_train_losses,name_loss="X-Entropy",save_dir=self.save_dir,name_file=f"_eval_train_{self.evaluated_epoch}_transfert_fine_{self.datamodule.name}")
            return  loss_ce
            ...
            if self.current_epoch==0 and batch_idx==0:
                ...
                save_attention_maps(img[:10],att_map_cls[:10].unsqueeze(1),att_map_spatial[:10].unsqueeze(1),self.current_epoch+1, self.save_dir,f"eval_{self.evaluated_epoch}_transfert_fine_{self.datamodule.name}")
                save_attention_maps_batch(att_map_cls=att_map_cls,att_map_spatial=att_map_spatial,epoch=self.current_epoch+1, output_dir=self.save_dir,name=f"eval_{self.evaluated_epoch}_transfert_fine_{self.datamodule.name}")
    ...
    def on_validation_epoch_end(self):
        ...
        if (self.current_epoch+1)%10 == 0 and not isinstance(self.performance[self.current_epoch+1],np.ndarray):
            plot_performance(list(self.performance.keys()),list(self.performance.values()),self.save_dir,name=f"val_final_{self.evaluated_epoch}_transfert_fine_{self.datamodule.name}")

    def on_fit_end(self):
        # Write to a CSV file
        with open(os.path.join(self.save_dir,f'performance_final_{self.evaluated_epoch}_transfert_fine_task_{self.task}_{self.datamodule.name}.csv'), 'w', newline='') as csvfile:
        ...
        if len(self.f1scores.keys()) > 1 :
            with open(os.path.join(self.save_dir,f'f1scores_final_{self.evaluated_epoch}_transfert_fine_task_{self.task}_{self.datamodule.name}.csv'), 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['Eval Epoch', 'Test Accuracy'])
